...
import ShortcutTrainerGame from "@/components/shortcut-trainer/ShortcutTrainerGame";
import ShortcutGuideCard from "@/components/shortcut-trainer/ShortcutGuideCard";
...
export const metadata = {
  title: "Latihan Kombinasi Shortcut Keyboard (Ctrl, Alt, Shift) - Game Hotkey Master",
  description: "Simulasi interaktif melatih kombinasi tombol shortcut (Ctrl, Alt, Shift, Win), skenario kerja nyata Microsoft Office, dan navigasi teks tanpa mouse untuk pemula serta siswa kursus komputer.",
};

export default async function LatihanShortcutPage() {
  ...
  const waLink = formatWhatsAppUrl(
    globalWhatsapp,
    "Halo Admin GWA Tech, saya telah mencoba Simulasi Latihan Shortcut Keyboard dan ingin bertanya info kursus komputer lebih lanjut."
  );
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="bg-amber-300 border-2 border-black px-2.5 py-1 text-[11px] font-mono font-bold shadow-[2px_2px_0px_0px_#000] rounded-md flex items-center gap-1.5">
                ...
                <span>Modul Shortcut & Navigasi</span>
              </span>
              <Link
                href="/siswa/login"
          ...
          <div className="bg-white border-3 border-black shadow-[6px_6px_0px_0px_#000] rounded-xl p-5 sm:p-6 relative overflow-hidden">
            ...
            <div className="max-w-3xl space-y-2 relative z-10">
              <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-amber-300 border border-black rounded text-[11px] font-mono font-black uppercase shadow-[1px_1px_0px_0px_#000]">
                <span>⚡ Latihan Refleks & Efisiensi Keyboard</span>
              </div>
              <h1 className="font-heading font-black text-2xl sm:text-3xl text-black tracking-tight leading-snug">
                Latihan Kombinasi Shortcut Keyboard (Office Ninja & Text Navigator)
              </h1>
              <p className="text-xs sm:text-sm text-slate-700 font-medium leading-relaxed">
                Tingkatkan efisiensi kerja Anda hingga 10x lipat dengan menguasai kombinasi tombol <strong>Ctrl, Alt, Shift, dan Win</strong>. Selesaikan misi skenario darurat kantor di Mode <em>Office Ninja</em> dan taklukkan navigasi seleksi teks profesional tanpa mouse di Mode <em>Text Navigator</em>.
              </p>
            </div>
          </div>
        </div>

        {/* ── INTERACTIVE SHORTCUT ENGINE ────────────────── */}
        <section aria-label="Game Latihan Kombinasi Shortcut Keyboard Interaktif">
          <ShortcutTrainerGame />
        </section>

        {/* ── EDUCATIONAL GUIDE CARD ────────────────── */}
        <section aria-label="Panduan & Lembar Contekan Shortcut">
          <ShortcutGuideCard />
        </section>

        {/* ── CTA TO JOIN COURSES ────────────────── */}
        <div className="bg-gradient-to-r from-amber-300 via-orange-300 to-cyan-300 border-3 border-black shadow-[8px_8px_0px_0px_#000] rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-center md:text-left">
            ...
            <h3 className="font-heading font-black text-xl sm:text-2xl text-black">
              Ingin Menjadi Mahir Microsoft Office & Komputer Perkantoran?
            </h3>
            <p className="text-xs sm:text-sm text-slate-800 font-medium max-w-xl">
              Daftar kursus komputer di {brandName} sekarang. Belajar praktikum langsung dengan instruktur profesional, kurikulum terstruktur, sertifikat resmi, dan fasilitas lab komputer lengkap.
            </p>
          </div>
