-- ============================================================
-- SQL Migration: Tabel landing_settings untuk CMS Halaman Utama
-- Jalankan ini di Supabase SQL Editor
-- ============================================================

-- 1. Buat Tabel landing_settings
...
-- 3. Kebijakan RLS: Siapapun (publik) dapat membaca (SELECT) konten landing page
DROP POLICY IF EXISTS "Public can view landing_settings" ON landing_settings;
CREATE POLICY "Public can view landing_settings"
  ON landing_settings
  FOR SELECT
  USING (true);

-- 4. Kebijakan RLS: Admin / API dapat mengelola data landing_settings
DROP POLICY IF EXISTS "Allow all modifications for landing_settings" ON landing_settings;
CREATE POLICY "Allow all modifications for landing_settings"
  ON landing_settings
  FOR ALL
  USING (true)
  WITH CHECK (true);
